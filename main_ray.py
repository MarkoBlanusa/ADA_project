...
import utils
from trade_env_ray import TradingEnvironment
...
def create_and_split_sequences(
    data_dict, input_length, validation_pct, test_pct, base_freq="4H"
):
...
def create_and_split_sequences_static(
    ...
    test_pct,
    base_freq="4H",
    num_action_state_features=5,  # Number of action-state features to include
):
    # Resample all timeframes to the base frequency of 15 minutes
    resampled_data = {
        tf: resample_to_frequency(df, base_freq) for tf, df in data_dict.items()
...
def env_creator(env_config):
    return TradingEnvironment(
        ...
        input_length=env_config.get("input_length", 84),
    )
...
if __name__ == "__main__":
    ...
    input_length = 84  # Define the length of the input window
    ...
    register(
        id="trading_env_ray-v0",
        entry_point="trade_env_ray:TradingEnvironment",
    )
    ...
    register_env("trading_env_ray-v0", env_creator)
    ...
    config.rollouts(
        ...
        rollout_fragment_length=2193,
        batch_mode="complete_episodes",
    )
    config.training(
        gamma=0.99,
        lr=1e-1,
        train_batch_size=8770,
        sgd_minibatch_size=840,
        num_sgd_iter=30,
    ...
    results = tune.run(
        ...
        # scheduler=scheduler,
        verbose=1,
        checkpoint_freq=20,  # Save a checkpoint every 10 training iterations
