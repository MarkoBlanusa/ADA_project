...
class TradingEnvironment(gymnasium.Env):
    ...
    def __init__(
        ...
        # self.batch_consumed_counter = 0
        # self.iteration_count = 0
        self.data_iterator = iter(
            self.normalized_data.iter_batches(batch_size=1, prefetch_batches=2048)
        )
        # # Initial state
        self.state = next(self.data_iterator)["data"][0]
        # self.iterator = self.normalized_data.iter_torch_batches(
        #     batch_size=1, prefetch_batches=1000
        # )
    ...
    def step(self, action):
        ...
        try:
            next_state = next(self.data_iterator)["data"][0]  # Move to the next batch
