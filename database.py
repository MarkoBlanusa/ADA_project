...
class Hdf5client:
    ...
    def get_data2(self, symbol: str, from_time: int, to_time: int, tf: str) -> Union[None, pd.DataFrame]:
        ...
        df = pd.DataFrame(data, columns=["timestamp", "open", "high", "low", "close", "volume"])
        df = df[(df["timestamp"] >= (from_time - params*1000)) & (df["timestamp"] <= (to_time - params*1000))]

        df["timestamp"] = pd.to_datetime(df["timestamp"].values.astype(np.int64), unit="ms")
        df.set_index("timestamp", drop=True, inplace=True)

        df = utils.resample_timeframe(df, tf)
